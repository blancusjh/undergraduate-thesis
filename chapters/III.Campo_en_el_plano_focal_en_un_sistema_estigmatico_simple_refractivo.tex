...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\textwidth]{figures/geometrical_setups/EGs.pdf}
...
donde $\xi$ está definida en \eqref{eq:xi_definition_fresnel_series}. Así, $t_+(0)=4n_0/(n_0+n_i)\neq 0$ mientras que $t_-(0)=0$: el coeficiente de mezcla es nulo sobre el eje y crece cuadráticamente alejándose de él. A esto se añade que $J_2(0)=0$, de modo que tanto el coeficiente $t_-$ como la función de Bessel $J_2$ se anulan simultáneamente en el origen, suprimiendo la conversión de helicidad en el eje. En contraste, $J_0(0)=1$ y $t_+(0)\neq 0$ garantizan que el término diagonal sí contribuye en el eje, generando un brillo central en el plano focal.

Esto se confirma en las figuras~\ref{fig:circ_10} y~\ref{fig:circ_01}, que muestran la propagación de $\ket{L}$ puro y $\ket{R}$ puro respectivamente: en el plano focal domina el canal de la misma helicidad con un disco de Airy con centro brillante, mientras que el canal de helicidad convertida es varios órdenes de magnitud menor y presenta mínimo central. Para entradas de polarización mixta $(1,1)$ y $(1,i)$ (figs.~\ref{fig:circ_11} y~\ref{fig:circ_1i}), ambas helicidades propagan discos de Airy prácticamente iguales de forma independiente; la diferencia de fase entre los casos no altera los módulos en el plano focal.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/circular/10/circular_10_abs_incident.png}\\[4pt]
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/circular/10/circular_10_abs_propagated.png}
  \caption{Simulación numérica para incidencia de polarización circular izquierda pura $(E_L,E_R)=(1,0)$. \textit{Arriba}: campo incidente; $E_L$ ocupa uniformemente la pupila, $E_R=0$, $E_z$ es no nulo. \textit{Abajo}: campo en el plano focal; $E_L'$ reproduce un disco de Airy con centro brillante (término $\mathscr{H}_0[t_+]$), $E_R'$ es varios órdenes de magnitud menor y con mínimo central (doble supresión por $J_2(0)=0$ y $t_-(0)=0$).}
  \label{fig:circ_10}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/circular/01/circular_01_abs_incident.png}\\[4pt]
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/circular/01/circular_01_abs_propagated.png}
  \caption{Simulación numérica para incidencia de polarización circular derecha pura $(E_L,E_R)=(0,1)$. El resultado es simétrico al caso $(1,0)$: $E_R'$ genera el disco de Airy dominante y $E_L'$ es el canal de conversión de helicidad, suprimido axialmente.}
  \label{fig:circ_01}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/circular/11/circular_11_abs_incident.png}\\[4pt]
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/circular/11/circular_11_abs_propagated.png}
  \caption{Simulación numérica para incidencia en superposición equitativa $(E_L,E_R)=(1,1)$. En el plano focal ambas helicidades propagan discos de Airy de igual amplitud de forma independiente, resultado consistente con el carácter diagonal del término $\mathscr{H}_0[t_+]$ dominante.}
  \label{fig:circ_11}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/circular/1i/circular_1i_abs_incident.png}\\[4pt]
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/circular/1i/circular_1i_abs_propagated.png}
  \caption{Simulación numérica para incidencia en superposición con desfase $(E_L,E_R)=(1,i)$. Las distribuciones de intensidad en el plano focal son idénticas al caso $(1,1)$ de la figura~\ref{fig:circ_11}; la diferencia de fase $\pi/2$ modifica el estado de polarización de la superposición pero no los módulos de las componentes.}
  \label{fig:circ_1i}
\end{figure}
...
Es claro como en esta representación la transformación que sufre el campo al refractarse en la interfaz $\Sigma$ es diagonal, esto es que los modos radiales y azimutales de la polarización son modos propios de este proceso de refracción a través de una superficie. La modulación radial está dada por la transformada de Hankel de orden 1 de los coeficientes de Fresnel; dado que la transformada de Hankel de orden uno es una integral ponderada por la función de Bessel $J_1$, no hay contribución central del campo en el eje óptico (figs.~\ref{fig:polar_10} y~\ref{fig:polar_01}). Esto es consecuencia de la homogeneidad del campo en la polarización radial y azimutal.

Este resultado, el que \textit{un campo radial u azimutal simétrico que sea axialmente simétrico, en su propagación hasta el plano focal, da lugar a un campo sin contribución central}.

Una diferencia notable entre los dos modos puros es la presencia de la componente longitudinal $E_z$: el modo radial genera una componente $E_z$ apreciable en la pupila (fig.~\ref{fig:polar_10}, arriba), consecuencia de la condición de transversalidad en un frente de onda convergente; el modo azimutal es intrínsecamente transversal y su componente $E_z$ es nula en la pupila (fig.~\ref{fig:polar_01}, arriba).

Para campos incidentes que son superposición de ambos modos, como los casos $(1,1)$ y $(1,i)$ mostrados en las figuras~\ref{fig:polar_11} y~\ref{fig:polar_1i}, cada componente se propaga de forma independiente por el carácter diagonal de la transformación. Los casos $(1,1)$ y $(1,i)$ son indistinguibles en intensidad pues el desfase relativo $\pi/2$ entre las componentes no altera los módulos $|E_r'|$ ni $|E_\phi'|$ en el plano focal.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/polar/10/polar_10_abs_incident.png}\\[4pt]
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/polar/10/polar_10_abs_propagated.png}
  \caption{Simulación numérica para incidencia de polarización radial pura $(\mathscr{E}_r,\mathscr{E}_\phi)=(1,0)$. \textit{Arriba}: campo incidente sobre la pupila; $E_r$ ocupa uniformemente el disco, $E_\phi=0$, $E_z$ es no nulo por la condición de transversalidad. \textit{Abajo}: campo en el plano focal; $E_r'$ forma un anillo con centro oscuro ($J_1(0)=0$), $E_\phi'=0$ se conserva, $E_z'$ presenta un patrón anular.}
  \label{fig:polar_10}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/polar/01/polar_01_abs_incident.png}\\[4pt]
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/polar/01/polar_01_abs_propagated.png}
  \caption{Simulación numérica para incidencia de polarización azimutal pura $(\mathscr{E}_r,\mathscr{E}_\phi)=(0,1)$. \textit{Arriba}: campo incidente sobre la pupila; $E_r=0$, $E_\phi$ ocupa uniformemente el disco, $E_z=0$ pues el modo azimutal es intrínsecamente transversal. \textit{Abajo}: campo en el plano focal; $E_\phi'$ forma un anillo con centro oscuro, mientras que $E_r'$ y $E_z'$ son despreciables.}
  \label{fig:polar_01}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/polar/11/polar_11_abs_incident.png}\\[4pt]
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/polar/11/polar_11_abs_propagated.png}
  \caption{Simulación numérica para incidencia en superposición equitativa de modos radial y azimutal $(\mathscr{E}_r,\mathscr{E}_\phi)=(1,1)$. \textit{Arriba}: campo incidente; $E_r$ y $E_\phi$ llenan uniformemente la pupila con igual amplitud, $E_z$ es no nulo. \textit{Abajo}: campo en el plano focal; ambas componentes forman patrones anulares independientes con centro oscuro.}
  \label{fig:polar_11}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/polar/1i/polar_1i_abs_incident.png}\\[4pt]
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/polar/1i/polar_1i_abs_propagated.png}
  \caption{Simulación numérica para incidencia en superposición con desfase $(\mathscr{E}_r,\mathscr{E}_\phi)=(1,i)$. Las distribuciones de intensidad son idénticas al caso $(1,1)$ de la figura~\ref{fig:polar_11}; el desfase relativo $\pi/2$ entre los modos no altera los módulos de las componentes en el plano focal.}
  \label{fig:polar_1i}
\end{figure}
...
La consecuencia más visible de $\mathbb{W}(\varphi)$ es la aparición de una componente cruzada en el plano focal con dependencia azimutal de orden dos. Para una entrada puramente $\hat{\mathbf x}$ (fig.~\ref{fig:cart_10}), el término $-\sin 2\varphi\,\mathscr{H}_2[t_-\mathscr{E}_x]$ genera una componente $E_y'$ con patrón de cuatro lóbulos, varios órdenes de magnitud menor que $E_x'$; el caso $\hat{\mathbf y}$ (fig.~\ref{fig:cart_01}) es simétrico. Para entradas de polarización mixta $(1,1)$ y $(1,i)$ (figs.~\ref{fig:cart_11} y~\ref{fig:cart_1i}), las contribuciones de la mezcla en $E_x'$ y $E_y'$ se compensan azimutalmente y ambas componentes presentan discos de Airy de amplitud similar.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/cartesian/10/cartesian_10_abs_incident.png}\\[4pt]
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/cartesian/10/cartesian_10_abs_propagated.png}
  \caption[Simulación numérica de incidencia lineal en x (1,0)]{Simulación numérica para incidencia de polarización lineal $\hat{\mathbf x}$ pura $(\mathscr{E}_x,\mathscr{E}_y)=(1,0)$. \textit{Arriba}: campo incidente; $E_x$ ocupa uniformemente la pupila, $E_y=0$, $E_z$ presenta una distribución dipolar. \textit{Abajo}: campo en el plano focal; $E_x'$ muestra un disco de Airy con centro brillante (término $\mathscr{H}_0[t_+]$); $E_y'$, varios órdenes de magnitud menor, presenta un patrón de cuatro lóbulos generado por el término $-\sin 2\varphi\,\mathscr{H}_2[t_-]$ de $\mathbb{W}(\varphi)$.}
  \label{fig:cart_10}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/cartesian/01/cartesian_01_abs_incident.png}\\[4pt]
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/cartesian/01/cartesian_01_abs_propagated.png}
  \caption[Simulación numérica de incidencia lineal en y (0,1)]{Simulación numérica para incidencia de polarización lineal $\hat{\mathbf y}$ pura $(\mathscr{E}_x,\mathscr{E}_y)=(0,1)$. El resultado es simétrico al caso $(1,0)$: $E_y'$ genera el disco de Airy dominante y $E_x'$ presenta el patrón de cuatro lóbulos de la mezcla por $\mathbb{W}(\varphi)$.}
  \label{fig:cart_01}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/cartesian/11/cartesian_11_abs_incident.png}\\[4pt]
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/cartesian/11/cartesian_11_abs_propagated.png}
  \caption[Simulación numérica de incidencia mixta (1,1)]{Simulación numérica para incidencia en superposición equitativa $(\mathscr{E}_x,\mathscr{E}_y)=(1,1)$. En el plano focal ambas componentes presentan discos de Airy de amplitud similar; las contribuciones de mezcla de $\mathbb{W}(\varphi)$ se compensan azimutalmente.}
  \label{fig:cart_11}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/cartesian/1i/cartesian_1i_abs_incident.png}\\[4pt]
  \includegraphics[width=\textwidth]{figures/results_of_simulation/first/cartesian/1i/cartesian_1i_abs_propagated.png}
  \caption[Simulación numérica de incidencia con desfase (1,i)]{Simulación numérica para incidencia con desfase $(\mathscr{E}_x,\mathscr{E}_y)=(1,i)$ (polarización circular en la base cartesiana). Las distribuciones de intensidad en el plano focal son análogas al caso $(1,1)$; la diferencia de fase no altera los módulos de las componentes.}
  \label{fig:cart_1i}
\end{figure}
