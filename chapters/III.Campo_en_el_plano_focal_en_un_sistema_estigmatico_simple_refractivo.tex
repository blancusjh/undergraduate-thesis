...
\end{equation}
donde $\mathscr R_n[f]$ denota el operador de propagación que toma un campo en un plano dado y da el campo en otro plano paralelo y a una distancia $f$ al primero en un medio de índice de refracción $n$. Tomando como válida en nuestro estudio la aproximación de Fresnel \cite{goodman2005}, que hemos usado implícitamente cuando transferimos los campos del plano al dioptrio y viceversa, tenemos que el operador de propagación está dado por
\begin{equation}
...
\end{align}
La figura~\ref{fig:sphere_theta_phi_fields} ilustra esta identificación: sobre la esfera de referencia centrada en $A'$, los modos de polarización $p$ y $s$ no son direcciones fijas sino los campos vectoriales tangentes $\uvec{\theta}_{A'}$ y $\uvec{\phi}$, de carácter meridional y azimutal respectivamente.

\begin{figure}[H]
  \centering
  \thesisincludegraphics[width=0.65\textwidth]
    {figures/geometrical_setups/esfera_polarizacion_rotada.pdf}
    {figures/fast/esfera_polarizacion_rotada.png}
  \caption{Campos vectoriales tangentes sobre la esfera de referencia centrada en el foco $A'$: (a)~parametrización de la dirección $\uvec{k}(\theta,\varphi)$; (b)~campo meridional $\uvec{\theta}$; (c)~campo azimutal $\uvec{\phi}$. El modo de polarización $p$ corresponde al campo meridional $\uvec{\theta}_{A'}$ y el modo $s$ al campo azimutal $\uvec{\phi}$.}
  \label{fig:sphere_theta_phi_fields}
\end{figure}

Pasamos pues, sin pérdida de generalidad, al problema transversal; denotamos $\vb E'_\perp$ como la parte transversal de $\vb E'$. De \eqref{eq:E_Sigma_Prime_in_terms_of_E_Sigma} se sigue que
\begin{equation}
\vb{E}'_\perp(\vb r,z)
=
n\frac{e^{inkz}}{i\lambda z}\,
...
\end{align}
Sustituyendo la normal \eqref{eq:N_ovoid_rho_phi_app} y los vectores unitarios \eqref{eq:u_for_P_in_optical_axis_app} y \eqref{eq:u_prime_for_P_in_optical_axis_app}, se obtiene
\begin{align}
\cos\theta_i(\rho)
&=
...
Con los cosenos de los ángulos de incidencia y refracción determinados en función de los paramétros de la superficie y la longitud de los rayos incidente y refractado, se procede a el cálculo de los coeficientes de Fresnel.
...
Con la geometría del dioptrio y el campo incidente ya caracterizados, estamos en condiciones de particularizar el resultado general de la sección~\ref{sec:caso_general} al sistema estigmático con fuente puntual en $A$. Reemplazando el campo incidente \eqref{eq:initial_field_in_p} en \eqref{eq:E_field_as_FT}, se tiene que
\begin{equation}
\vb{E}'_\perp(\vb r,z)
=
n\frac{e^{inkz}}{i\lambda z}\,
...
\begin{figure}[H]
  \centering
  \thesisincludegraphics[width=0.65\textwidth]
    {figures/geometrical_setups/esferas_GGp.pdf}
    {figures/fast/esferas_GGp.jpg}
...
\begin{equation}
    ...
    \mathcal{O}(r^4),
\end{equation}
donde $O$ es uno de los parámetros de la parametrización GOTS de las superficies cartesianas que se desarrolló en \cite{singlet2020}, cuyas definiciones escribimos en el presente trabajo en el apéndice \ref{apx:ovoid_implicit_deduction}, y el desarrollo en series de la sagita, $\ell_0$, entre otras cantidades relevantes, se realiza con más detalle en el apéndice \ref{apx:ovoids_series}. Asumiendo que el índice de incidencia es $n_0=1$, el índice de salida es $n_i=n$, y la posición de la imagen estigmática es $z_i=f$, el parámetro $O$ está dado por
\begin{equation}
    O
    =
    \frac{n_i z_0 - n_0 z_i}{z_i z_0 (n_i - n_0)}
    =
    \frac{1}{n - 1}
    \left(
    \frac{n}{f}
...
Es evidente a partir de este resultado que, cuando la distancia de observación $z$ coincide con el foco del sistema, $z=f$, el término cuadrático en $r$ de la fase se cancela exactamente. Esto significa que la fase de la pupila se vuelve constante, $\Phi\simeq -z_0$, y podemos hacerla $1$ sin pérdida de generalidad. Reemplazando la fase de pupila por una fase constante en la ecuación \eqref{eq:E_field_perp_no_phase_replacement}, obtenemos
\begin{equation}
  \vb E'(\vb r, f)
  =
...
\end{equation}
y, sobre la esfera de Fourier $\mathscr D$ de $\mathscr G'$, libre del factor de fase cuadrático,
\begin{equation}
  \vb E'^{\mathscr D}(\vb r)
  =
  ...
  \label{eq:E_D_in_EGp}
\end{equation}
Usando $\mathbb E^{\mathscr G'} = \mathbb T \mathbb E_0 / \ell_0$ y aproximando $\ell_0 \approx z_0$ en la amplitud, estas se leen equivalentemente como
\begin{equation}
\boxed{
  \vb E'(\vb r, f)
  =
...
\boxed{
  \vb E'^{\mathscr D}(\vb r)
  =
  \frac{ne^{inkf}}{i\lambda f z_0}  \,\mathscr F\!\left\{\mathbb T\mathbb E_0\right\}\!\left(\frac{n}{\lambda f}\vb r\right).
}
...
Wolf el operador vectorial se postula desde la construcción aplanática, pues
asume una lente ideal que satisface la condición de los senos de Abbe~\cite{Abbe1873}, y
...
construye punto a punto desde la geometría de la superficie
...
introducida por $t_-$. Si bien el caso de reflexión no es tratado en detalle en el presente trabajo, los resultados son en esencia los mismos: bastaría con reemplazar los coeficientes de Fresnel de refracción por los de reflexión.
...
modulación espacial es paralela a $\uvec{x}$ ($C_V$), y otro a través de
trazas horizontales, con modulación paralela a $\uvec{y}$ ($C_H$). El
...
$\uvec{x}$, las trazas alineadas con la polarización conservan la mayor
...
$\uvec{y}$ los papeles se intercambian ($C_V=0.81$, $C_H=0.68$): los
...
\begin{figure}[H]
  ...
  iluminación lineal $\uvec{x}$ (izquierda) y $\uvec{y}$
...
polarizaciones $\uvec{p}$ y $\uvec{s}$, orientadas radial y
...
óptico \cite{mcguire1994}. Que se basa en escribir la matriz que caracteriza la transformalción del campo incidente en el campo sobre la esféra gaussiana de referencia centrada en el punto imagén como una combinación lineal de las matrices de Pauli. La analogía con la teoría de aberraciones de frente
de onda es directa: así como la aberración escalar se define como una función
de fase sobre la esfera de referencia de salida, la aberración vectorial se describe mediante la función
...
compacta mediante la coordenada proyectiva compleja \cite{azzam1977}
\begin{equation}
\zeta(\rho',\phi')
=
...
transversales del campo sobre $\mathscr{G}$. El ángulo de amplitudes
$\vartheta$, definido por $\tan\vartheta=|\mathscr E_y|/|\mathscr E_x|$, mide
la razón entre las amplitudes de las dos componentes cartesianas, mientras que
$\delta=\arg\mathscr E_y-\arg\mathscr E_x$ es la fase relativa entre ellas.
Estos dos parámetros determinan por completo la elipse de polarización local:
$\vartheta$ fija el reparto de amplitud entre las componentes y $\delta$ la
elipticidad y el sentido de recorrido. Nótese que $\zeta$ es insensible
...
transmisión. De $\zeta_{\mathscr{G}'}$ se obtienen el ángulo de amplitudes y
la fase relativa del estado transmitido,
\begin{equation}
...
\end{equation}
Esta ley de transformación es suficiente para determinar el campo transmitido:
conocida la componente $E_x^{\mathscr{G}'}$, la componente restante queda dada
por $E_y^{\mathscr{G}'}=\zeta_{\mathscr{G}'}\,E_x^{\mathscr{G}'}$, con lo que
el campo sobre $\mathscr{G}'$ queda completamente caracterizado.
...
(\mathbb{T}_{xy}\mathbb E_0)_x.
\end{equation*}
Sustituyendo en \eqref{eq:zeta_focal} la forma explícita
\eqref{eq:TxyE0_components} de $(\mathbb{T}_{xy}\mathbb E_0)_x$, donde el
factor común $1/2$ se cancela, se obtiene
\begin{equation}
\zeta_f(\vb r)
=
\frac{
...
La ecuación \eqref{eq:zeta_focal_mobius} incorpora de manera explícita la
transformación local \eqref{eq:mobius_transformation} en el resultado focal.
De ella se recuperan, en cada punto de observación, la razón entre las
amplitudes y la fase relativa,
$\tan\vartheta_f=|\zeta_f|$ y $\delta_f=\arg\zeta_f$, y con ellas, junto con
la componente $E'_x$, el campo en el plano focal.
...
Para proceder con los resultados en cada representación, se parte de la ecuación \eqref{eq:E_field_pupil_phase_and_TT} junto con la definición \eqref{eq:E_G_prime_def}, y escribiremos tanto $\mathscr E_\theta$, $\mathscr E_\phi$ como los vectores unitarios $\uvec s$  y $\uvec p'$ en la representación deseada. En la presente sección tendremos en cuenta las siguientes definiciones. Definimos la transformada de Hankel de orden $m$ como
\begin{equation}
\mathscr H_m[f](q)
=
\int_0^\infty
...
\end{equation}
donde $q$ es la frecuencia radial conjugada a $r'$ en la transformada de Fourier bidimensional. También usaremos las combinaciones $t_\pm = t_p \pm t_s$ introducidas en
\eqref{eq:t_plus_t_minus_def}. Para el caso de una pupila circular de radio $a$, definimos la función pupila
\begin{equation}
A_a(r')
=
\begin{cases}
...
La ecuación \eqref{eq:E_prime_in_circular_representation} puede simplificarse notablemente si asumimos que las componentes $\mathscr E_L$ y $\mathscr E_R$ presentan simetría axial. Además, son de interés en óptica, pues campos escritos de esa manera representan haces vectoriales cilíndricos, entre los que los haces radiales, azimutales y espirales son casos particulares, y corresponden a nuestro caso de estudio. Como ya tenemos claro, el campo transmitido en el plano focal es proporcional a
\begin{equation}
...
\qquad (m\in\mathbb Z)
\end{equation}
que es la expansión de Jacobi-Anger con un cambio de variable \cite{Watson1944}, de la cual se obtiene que
\begin{equation}
...
Este resultado es todavía general: no se ha supuesto que las componentes escalares $\mathscr E_r$ y $\mathscr E_\phi$ sean axialmente simétricas.

\subsubsection{Reducción del problema a componentes axialmente simétricas}

Supongamos ahora que las componentes cilíndricas del campo incidente no dependen de $\phi'$, es decir,
\begin{equation}
...
Para efectuar la integral angular usamos la identidad de Jacobi--Anger \cite{Watson1944},
\begin{equation}
  \int_0^{2\pi}
  e^{im\phi'}
  e^{-iq\rho'\cos(\phi'-\varphi)}
...
\end{equation}
Nótese que lo que se transforma de esta forma no es el operador diádico $\mathbb T$ por sí solo, sino el campo transmitido $\mathbb T\mathbb E_0$. Sustituyendo \eqref{eq:Fphi_rhat} y \eqref{eq:Fphi_phihat} en \eqref{eq:E_prime_cylindrical_general}, se obtiene
\begin{equation}
\boxed{
  \vb E'_\perp(\vb r,f)
  =
...
Podemos decir que \textit{un campo radial o azimutal simétrico que sea axialmente simétrico, en su propagación hasta el plano focal, da lugar a un campo sin contribución central}, resultado bien conocido en la literatura de haces vectoriales cilíndricos \cite{Youngworth2000, Zhan2009}.
...
\begin{equation}
  ...
  \right),
\end{equation}
junto con la identidad de Jacobi--Anger \cite{Watson1944},
\begin{equation}
\int_0^{2\pi}
e^{im\phi'}
e^{-iq\rho'\cos(\phi'-\varphi)}
...
\begin{figure}[H]
  ...
  \caption{Mapas de polarización para incidencia lineal $\uvec{x}$: campo sobre la pupila (izquierda) y campo en el plano focal (derecha). El fondo muestra $|E_x|^2+|E_y|^2$ y las flechas la orientación local de la polarización, evidenciando la rotación inducida por la componente cruzada.}
...
\begin{figure}[H]
  ...
  \caption{Ángulos de la elipse de polarización para incidencia lineal $\uvec{x}$: elipticidad $\chi$ (izquierda) y orientación del eje mayor $\psi$ (derecha), sobre la pupila (fila superior) y en el plano focal (fila inferior). La elipticidad es nula en todo el plano focal: el estado permanece lineal y la aberración vectorial se reduce a una rotación local de la orientación. Las regiones en blanco corresponden a zonas de intensidad demasiado baja, donde estos parámetros dejan de estar bien definidos.}
...
incidencia lineal $\uvec{x}$ con amplitud axialmente simétrica
...
La figura~\ref{fig:spot-cortes-principales} compara el perfil focal
vectorial con la referencia escalar ideal ($t_p=t_s=1$) para el dioptrio de
vidrio usado en los resultados anteriores, a apertura alta: la transmisión
vectorial de Fresnel ensancha el lóbulo central respecto de la predicción
escalar. La deformación anisótropa crece con la apertura y con el contraste
de índice, pues ambos incrementan el canal $H_2$.

\begin{figure}[H]
  \centering
  \thesisincludegraphics[width=0.7\textwidth]
    {figures/results_of_simulation/vecdiff_final/spot/spot_cortes_principales.png}
    {figures/fast/spot_cortes_principales.jpg}
  \caption{Corte radial de la intensidad en el plano focal para incidencia
  lineal en el dioptrio de vidrio ($n_0=1$, $n_i=1.5$, $z_0=-10\,\mathrm{mm}$,
  $z_i=6\,\mathrm{mm}$, $\lambda=532\,\mathrm{nm}$, pupila uniforme a apertura
  alta). La transmisión vectorial de Fresnel ensancha el punto focal respecto
  de la referencia escalar ideal ($t_p=t_s=1$); a esta escala los cortes por
  los dos ejes principales son indistinguibles.}
  \label{fig:spot-cortes-principales}
\end{figure}

La figura~\ref{fig:spot-radios-anotados}
...
Para incidencia circular la intensidad es $I\propto|H_0|^2+|H_2|^2$, el
promedio azimutal del caso lineal: el punto permanece circular y solo se
ensancha levemente ($R=0.261\lambda$ en el mismo dioptrio), pues el canal
cruzado $|H_2|^2$ es un anillo nulo en el eje. Cabe señalar que estos
resultados corresponden a la intensidad transversal; a aperturas muy altas la
contribución longitudinal $I_z$, determinada por la condición de
transversalidad, modifica adicionalmente la forma del lóbulo total.
