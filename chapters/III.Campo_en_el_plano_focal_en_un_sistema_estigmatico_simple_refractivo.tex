...
\begin{align}
  \vb E'(\vb r, f)
  &=
  \frac{n}{i\lambda f z_0}\,e^{inkf}\,e^{i\frac{nk}{2f}r^2}
  \,\mathscr F\!\left\{\mathbb M\mathbb E_0\right\}\!\left(\frac{n}{\lambda f}\vb r\right),
  \label{eq:E_field_in_E0}
  \\
  \vb E^{\mathscr D}(\vb r)
  &=
  \frac{n}{i\lambda f z_0}\,e^{inkf}
  \,\mathscr F\!\left\{\mathbb M\mathbb E_0\right\}\!\left(\frac{n}{\lambda f}\vb r\right).
  \label{eq:E_D_in_E0}
\end{align}
...
Aplicando $\mathscr{F}_\phi$ a esa expresión y usando las identidad 
...
Cuando, adicionalmente, los coeficientes de Fresnel $t_p$ y $t_s$ se tratan como constantes sobre la apertura, la única integral adicional necesaria es
...
que se evalúa mediante la identidad $\int_0^{x} u\,J_0(u)\,du = x\,J_1(x)$. Combinando con \eqref{eq:H2_chiR} y sustituyendo en \eqref{eq:E_L_E_R_Hankel}, se obtiene el campo en el plano focal para una pupila y campo homogéneos:
...
e^{i\frac{nk}{2f}r^2}
\left[
(t_p+t_s)\frac{a}{q}J_1(qa)\,E_{L,0}
-
e^{2i\varphi}
(t_p-t_s)
\frac{2\bigl(1-J_0(qa)\bigr)-qa\,J_1(qa)}{q^2}
\,E_{R,0}
\right],
\\
E'_R(\vb r,f)
...
\,E_{L,0}
+
(t_p+t_s)\frac{a}{q}J_1(qa)\,E_{R,0}
\right].
\end{aligned}
\label{eq:E_L_E_R_homogeneous_exact}
\end{equation}
...
\frac{2\pi n}{i\lambda f}\,e^{inkf}\,e^{i\frac{nk}{2f}r^2}
\begin{pmatrix}
t_+\,P & -e^{2i\varphi}\,t_-\,Q
\\[4pt]
-e^{-2i\varphi}\,t_-\,Q & t_+\,P
\end{pmatrix}
\begin{pmatrix} E_{L,0} \\ E_{R,0} \end{pmatrix}.
\label{eq:E_L_E_R_homogeneous_matrix}
\end{equation}

El término diagonal, proporcional a $P(q)=\frac{a}{q}J_1(qa)$, preserva la helicidad incidente y corresponde al patrón de Airy. El término antidiagonal, proporcional a $Q(q)=\frac{2(1-J_0(qa))}{q^2}-\frac{a}{q}J_1(qa)$, acopla $\ket L \leftrightarrow \ket R$ con modulación azimutal $e^{\pm 2i\varphi}$ y es estrictamente nulo en el eje, pues $J_0(0)=1$ y $J_1(0)=0$.
