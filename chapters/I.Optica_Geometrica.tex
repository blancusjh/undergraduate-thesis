...
\begin{equation}
  f_{\Sigma}(x,y,z) = O G z^2 - 2(1 + S\rho^2)z + (O + T\rho^2)\rho^2 = 0 
  \label{eq:implicit_ovoid_GOTS}
\end{equation}

\begin{equation}
  z(\rho) = \frac{(O + T\varphi^2)\varphi^2}{1 + S\varphi^2 + \sqrt{1 + (2S - O^2G)\varphi^2}}
  \label{eq:parametrical_ovoid_z}
\end{equation}

\begin{equation}
  r(\varphi) = \sqrt{\rho^2 - z^2(\varphi)} 
...
\caption{Dioptrio cartesiano en condición fuera de eje y fuera de foco: el trazado en $r$--$z$ y el diagrama de manchas muestra como si la fuente no esta en un punto estigmatico no se obtiene en la pantalla un punto sino una macha extendida.}
