...
\begin{equation}
    \mathcal{R}_n [f] \{ U(x,y) \} = n\frac{e^{inkf}}{i \lambda f} \iint_{-\infty}^{\infty} U(x',y') e^{\frac{ink}{2z} \lVert \vb{r}-\vb{r}' \rVert^2} d\vb{r}'
\end{equation}
...
En terminos de una transformada de Fourier el campo $\vb E$ queda expresado como
...
Definase un sistema coordenado esférico centrado en el foco del sistema, que es el punto $A'$ , llámese por $\uvec R_{A'}, \uvec \theta_{A'}, \uvec \phi$ los vectores curvilíneos asociados al sistema. 


Los estados de polarización $\uvec p'$  y $\uvec s$, son la polarización definida por $\uvec \theta_{A'}$ y $\uvec \phi$ respectivamente. 

De modo que en el sistema esférico centrado en $A'$ la base de polarizacion $\hat s$, $\hat p$ queda escrita como 
...
Si bien esta base de polarización sugiere que de manera general el campo tiene una naturaleza trimidensional, el problema puede restringirse al plano perpendicular al eje óptico debido a que la componente en z puede ser reconstruida a partir del plano $xy$ haciendo uso de la condidicón de transversalidad
...
donde \(E_r\) es la componente del campo sobre \(\uvec{p}_\perp\). Por tanto,

\begin{equation}
E_z = -\tan\theta_{A'}\,E_r.
\label{eq:Ez_de_la_transversalidad}
\end{equation}
...
De tal manera que el problema que se resume en la ecuación  \ref{E_in_focal_plane_as_FT_3D}, puede reducirse a 
...
= -\sin\phi\,\uvec{x} + \cos\phi\,\uvec{y} = \uvec s, \\
...
Sustituyendo las expresiones explícitas de \(\uvec{p}_\perp\) y \(\uvec{s}_\perp\), se sigue que
\begin{align}
\uvec{e}_+
&=
\frac{1}{\sqrt{2}}
\left[
(\cos\phi - i\sin\phi)\uvec{x}
+
(\sin\phi + i\cos\phi)\uvec{y}
\right], \\[6pt]
\uvec{e}_-
&=
\frac{1}{\sqrt{2}}
...
\frac{\uvec{x} + i\,\uvec{y}}{\sqrt{2}},
\\[6pt]
\uvec{e}_-
&=
e^{i\phi}
\frac{\uvec{x} - i\,\uvec{y}}{\sqrt{2}}.
\label{eq:epm_usual_circular}
\end{align}

De esta manera, las bases \(\uvec{e}_+\) y \(\uvec{e}_-\) se expresan en términos de la base circular derecha e izquierda usual, salvo por un factor de fase azimutal \(e^{\mp i\phi}\), asociado a la rotación local de la base cilíndrica respecto de la base cartesiana.

En esta representación, el campo eléctrico se escribe como
\begin{equation}
\vb{E} = E_+ \uvec{e}_+ + E_- \uvec{e}_- + E_z \uvec{z}.
\label{eq:E_in_e_+-_basis}.
\end{equation}
...
% --- Problem In Terms of | L >, |R > Basis. 

Por conveniencia usaremos la base circular derecha e izquierda usuales definidas sobre el plano $xy$, las bases $\uvec s, \uvec p_\perp$ queda en esos terminos expresada como  
...
Remplazando en la ecuación $\eqref{eq:P_definition}$ %NOTE : REFERENCIAR.

\begin{equation}
  \vb{P}_\perp = \frac{1}{\sqrt{2}}e^{\,ik\left(\frac{r'^2}{2f} + (1-n)z(r')\right)}
\Big\{ 
...
\mathbf E(\mathbf r,f)=
\frac{e^{ikf}}{i\lambda f}
e^{i\frac{k}{2f}\|\mathbf r\|^2}
\frac{1}{\sqrt{2}}
\mathcal F\left\{
e^{ik\left(\frac{r'^2}{2f}+(1-n)z(r')\right)}
\left[
(t_pE_p+i t_sE_\phi)e^{i\phi}\ket L
+
(t_pE_p-i t_sE_\phi)e^{-i\phi}\ket R
\right]
\right\}
\left(\frac{\mathbf r}{\lambda f}\right).
\end{equation}
...
\right\}
\left(\frac{\mathbf r}{\lambda f}\right)\ket L \\
+ \;
&\mathcal F\left\{
e^{ik\left(\frac{r'^2}{2f}+(1-n)z(r')\right)}
(t_pE_p-i t_sE_\phi)e^{-i\phi}
\right\}
\left(\frac{\mathbf r}{\lambda f}\right)\ket R
\Bigg].
\end{align}
